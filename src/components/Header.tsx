...
export default function Header() {
    ...
    const handleLogoClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
        ...
        // We trigger a global event so the page content can animate out if it wants to,
        // or just wait 500ms before navigating
    ...
    return (
        <>
            {/* ── Fixed header bar ─────────────────────────────────────────── */}
            <header className="fixed top-0 left-0 w-full z-50 h-[107px] md:h-[147px] desktop:h-[113px] bg-[#FAFAFA]">
                <div className="h-full w-full max-w-[1440px] mx-auto relative flex flex-col items-center justify-center gap-3 md:gap-4 px-1.5 py-3 md:px-8 md:py-5 desktop:flex-row desktop:justify-center desktop:px-[102px] desktop:py-5">
                    ...
                    {/* Desktop nav — hidden below 1440px */}
                    ...
                    {/* Hamburger button — visible below 1440px, pinned top-right of the header */}
                    <button
           ...
           Slides in from the right exactly below the header.
           Dimensions from Figma: 151×112px mobile, 213×182px tablet.       ── */}
            <div
                className={`
          fixed right-0 z-40
          ...
          bg-[#ffffff]
          border-l border-b border-[#141414]
